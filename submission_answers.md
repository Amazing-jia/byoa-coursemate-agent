...
在工具（Skills）方面，我实现了多个核心工具：**read_local_file** 用于读取用户上传的课程文件，**rank_passages** 用于对提取出的文本片段进行混合检索和排序，**answer_with_external_api** 用于调用外部 OpenAI-compatible API 生成最终回答。项目还提供了一个中文网页界面，用户可以输入 API Key、检测 API 连接、自动获取可用模型列表，并上传多个课程资料文件。模型候选会合并 API 实际返回列表和平台预设列表，以减少部分平台模型接口返回不完整造成的遗漏。API Key 输入框会自动隐藏内容，检测成功后会记忆在本浏览器本地，避免每次提问重复输入。

在上下文（context）集成方面，我采用了基于函数调用的工具集成方式。Agent 会先读取用户上传的课程资料，再将资料切分为 slide/page/段落片段，并使用关键词、中文到英文术语映射和模糊匹配进行混合检索；如果用户提供 API Key，还可以用外部 AI 对候选片段进行轻量重排。最后，Agent 会把检索到的真实课程内容作为外部上下文传入 API，从而实现基于课程材料的问答，而不是只依赖模型自身知识。网页只展示最相关原文片段，方便验证回答来源，同时避免输出过长的完整课程原文。
