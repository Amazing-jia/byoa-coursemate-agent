...
- API Key 输入框使用密码模式隐藏内容，并可手动清除本地记忆
- API 设置可在浏览器本地记忆，避免每次提问重复输入
- 可检测 API 连接，并自动获取该接口返回的可用模型列表
- 模型候选会合并“接口真实返回列表”和“平台预设常用模型”，避免部分平台 `/models` 返回不完整时漏掉可用模型
- 支持关闭外部 AI，只查看本地检索结果
...
3. 输入问题。
4. 在右侧填写 API Key 和 API Base URL。
5. 点击“检测连接并获取模型”，连接成功后页面会记住 API 设置，并在模型名称处提供可用模型候选。
6. 点击“开始回答”。
7. 页面会显示 Agent 回答、检索到的课程上下文，以及最相关原文片段。

如果只想测试本地读取和检索功能，可以取消勾选“调用外部 AI”。

API Key 会保存在当前浏览器的 localStorage 中，方便课堂演示时连续提问。点击“清除记忆”可以删除本地保存的 API 设置。

如果某个平台的模型接口返回不完整，页面会继续显示该平台的预设模型候选。例如 DeepSeek 接口只返回少量模型时，候选框仍会保留 `deepseek-chat`、`deepseek-reasoner`、`deepseek-v4-flash`、`deepseek-v4-pro`、`deepseek-v4pro` 等可选项。候选框只是辅助选择，用户仍然可以直接手动输入服务商实际支持的模型名称。
