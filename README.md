...
本项目采用基于函数调用的上下文集成方式。Agent 会先调用本地文件读取工具提取课程资料文本，再调用检索工具找到和问题相关的内容，最后把检索结果作为外部上下文传入 AI API。

当用户上传多个文件时，Agent 会依次读取每个文件，将文件名和提取出的文本合并成可检索上下文。页面只展示最相关原文片段，用于证明回答来源，同时避免显示过长的完整资料。
